...
if __name__ == "__main__":
    ...
    def createNewMap(annoWarp, imageName, regions):
        y, x = annoWarp.shape
        mapImage = np.zeros((y, x, 3), dtype="uint8")
                ...
                if not all(x == area for x in surroundingPoints) and not all(
                    ...
                    try:
                        if not all(
                            ...
                            mapImage[j, i] = [255, 255, 255]
                    except:
                        pass
                try:
                    if all(x == area for x in surroundingPoints):
                        if "layer" in regions[area]["name"].lower():
                            regions[regions[area]["parent"]]["map_points"].append(
                                (j, i)
                            )
                        else:
                            regions[area]["map_points"].append((j, i))
                except:
                    pass
        ...
        for region, info in regions.items():
            if info["points"] != [] and region not in [997, 688, 1009, 0]:
                ...
                if args.whole:
                    m = np.mean(
                        [p for p in info["map_points"] if p[1] < (x) // 2], axis=0
                    ).astype(np.int32)
                else:
                    m = np.mean([info["map_points"]], axis=0).astype(np.int32)
                try:
                    cv2.putText(
                        mapImage,
        ...
        cv2.imwrite(
            str(args.input.strip() + "/" + f"Map_{imageName.split('.')[0]}.png"),
            mapImage,
        )
    ...
    for annoPkl, dapi in zip(annotationsPkl, dapiImages):
        ...
        if args.mode.strip().lower() == "affine":
            ...
            cv2.createTrackbar("Y Scale", "Scale and Position", 0, 100, adjustPosition)
            cv2.createTrackbar("X Scale", "Scale and Position", 0, 100, adjustPosition)
            ...
            cv2.createTrackbar("Rotation", "Scale and Position", 0, 360, adjustPosition)
                ...
                try:
                    if not drawing:
                        ...
                        if selectedRegion != None:
                            ...
                            cv2.putText(
                                newAdjustment,
                                regions[selectedRegion]["acronym"],
                                (10, 175),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.75,
                                (0, 0, 255),
                        ...
                        annoWarp[
                            y - brushSize : y + brushSize, x - brushSize : x + brushSize
                        ] = selectedRegion
            ...
            if key == ord("q"):
                ...
                with open(os.path.join(args.input, annoPkl), "wb") as f:
                    if args.mode.strip().lower() == "affine":
                        ...
                        annoTranslated = cv2.warpAffine(
                            annoWarp.astype("float64"),
                            annoM,
                            (annoCols, annoRows),
                    ...
                    if args.draw:
                        createNewMap(annoWarp, imageName, regions)
