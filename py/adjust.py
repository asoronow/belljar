...
parser.add_argument('-w', '--whole', help='whole brain mode',
                    action='store_true', default=False)
parser.add_argument(
    '-m', '--mode', help='mode to run in, either "paint" or "affine"', default='affine')
parser.add_argument('-d', "--draw", help="draw new maps",
                    action="store_true", default=False)
    ...
    def createNewMap(annoWarp, imageName, regions):
                    ...
                    if all(x == area for x in surroundingPoints):
                        if "layer" in regions[area]['name'].lower():
                            regions[regions[area]['parent']
                                    ]['map_points'].append((j-101, i-101))
                        else:
                            regions[area]['map_points'].append((j-101, i-101))
                except:
                    pass
        ...
        for region, info in regions.items():
            if info['points'] != [] and region not in [997, 688, 1009, 0]:
                ...
                if args.whole:
                    m = np.mean(
                        [p for p in info['map_points'] if p[1] < (x-200)//2], axis=0).astype(np.int32)
                else:
                    m = np.mean([info['map_points']], axis=0).astype(np.int32)
                try:
                    cv2.putText(mapImage, regions[region]['acronym'], (
                        m[1] - 2, m[0]), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1)
                except Exception as e:
                    pass

        cv2.imwrite(
            str(args.input.strip() + "/" + f"Map_{imageName.split('.')[0]}.png"), mapImage)
    ...
    with open(args.structures.strip()) as structureFile:
        ...
        for row in structureReader:
            regions[int(row[0])] = {"acronym": row[3], "name": row[2], "parent": int(
                row[8]), "points": [], "map_points": [], "color": getColor()}
    ...
    for annoPkl, dapi in zip(annotationsPkl, dapiImages):
        ...
        if args.mode.strip().lower() == 'affine':
            ...
            cv2.createTrackbar('X Scale', 'Scale and Position',
                               0, 100, adjustPosition)
            cv2.createTrackbar('X', 'Scale and Position',
                               0, 1000, adjustPosition)
            cv2.createTrackbar('Y', 'Scale and Position',
                               0, 1000, adjustPosition)
