...
class PolygonSelectionTool:
    ...
    def drawPolygonSelection(self, event,curr_x,curr_y,flags,param):
        ...
        if event==cv2.EVENT_LBUTTONDOWN:
            if self.firstPoint:
                ...
                cv2.circle( self.image, (self.last_x, self.last_y), 2, (0,0,255), 3 )
            elif not self.firstPoint:
                if abs(self.firstX-curr_x) <= 15 and abs(self.firstY - curr_y) <= 15:
                    cv2.polylines( self.image, np.int32([self.verts]), True, (0,0,255) ,2)
                else:
                    cv2.circle( self.image, (curr_x, curr_y), 2, (0,0,255), 3 )
                    cv2.line( self.image, (self.last_x,self.last_y), (curr_x,curr_y), (0,255,255) ,2 )
                    self.last_x,self.last_y=curr_x,curr_y
                    self.verts = np.append(self.verts, [[self.last_x, self.last_y]], axis=0)
    ...
    def startDrawing(self):
        ...
        cv2.fillPoly(self.mask, pts=np.int32([self.verts]), color= (255,255,255))
